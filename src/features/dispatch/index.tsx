...
import { BrokerDispatchView } from "./components/BrokerDispatchView";
...
// ── HR Dispatch (corporate-scoped key) ────────────────────────────────────────
// Unchanged from original — Preview available, Dispatch button hidden (Step 1).

function HrDispatchView() {
  ...
  return (
    <div className="flex h-full flex-col">
      <TopBar
        ...
        subtitle={
          isBroker
            ? "Manage dispatch queues across your entire corporate portfolio"
            : "Batch-send pending employees to the insurer and track report history"
        }
      />

      <div className="flex-1 overflow-y-auto p-6">
        {isBroker ? <BrokerDispatchView /> : <HrDispatchView />}
      </div>
    </div>
  );
}
